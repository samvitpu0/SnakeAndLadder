...
{
    /// <summary>
    /// Support for property editor based on AngryAnt example
    /// </summary>
    public abstract class PropertyEditorSupport : UnityEditor.Editor
    {
        protected new SerializedObject serializedObject;


        private static GUIStyle commentStyle = null;
        //private static bool cameraRendered = false; //commented by kashyap, as this is a requities of onSceneGUI, which is no longer in use
        ...
        protected void TexturePropertyField(SerializedProperty property, bool allowSceneObjects, params GUILayoutOption[] options)
        {
            Object newValue = EditorGUILayout.ObjectField(property.objectReferenceValue, typeof(Texture2D), allowSceneObjects, options);
        ...
        protected void EnumMaskPropertyField<T>(string label, SerializedProperty property) where T : struct, System.IConvertible
        {
            if (!typeof(T).IsEnum)
            {
                throw new System.ArgumentException("T must be an enum type");
            }

            Rect rect = GUILayoutUtility.GetRect(20.0f, 16.0f);
            GUIContent validLabel = EditorGUI.BeginProperty(rect, new GUIContent(label), property);

            EditorGUI.BeginChangeCheck();

            System.Enum value = EditorGUI.EnumFlagsField(rect, validLabel, (System.Enum) System.Enum.ToObject(typeof(T), property.intValue));
        ...
        {
            get { return ((Component) target).transform; }
        }
        ...
        //not implementing/override as this prevents drawing of unity UI elements -- kashyap
